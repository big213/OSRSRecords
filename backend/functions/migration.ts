...
export async function up(knex: Knex): Promise<void[]> {
  return Promise.all([
    ...
    knex.schema.createTable("eventGroup", function (table) {
      ...
      table.string("avatar").nullable();
      table.string("name").notNullable();
      table.jsonb("contents").notNullable().defaultTo([]);
    ...
    knex.schema.createTable("submission", function (table) {
      ...
      table.jsonb("participants_list").notNullable().defaultTo([]);
      ...
      table.boolean("is_relevant_record").notNullable().defaultTo(false);
